...
1. The primary contigs of ABS4 inbred assembly is generated by hifiasm with `-l 0` parameter (`-l 0` is for homozygosity assumption). `-u` command enables postjoining the overlapping region but create assembly errors else where, som we disabled it here.
    
2. Misassembled (a contig is wrongly connected by (TAC)n repeats) and over-scaffold (ragtag did not distinguish heterozygosity region) are resolved by custom-scripts by removing overlapping cotigs with former contigs.
