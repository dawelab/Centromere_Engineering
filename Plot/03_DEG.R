...
core_gene_set <- read.csv("https://raw.githubusercontent.com/dawelab/Natural-methylation-epialleles-correlate-with-gene-expression-in-maize/refs/heads/main/Data/B73.all.csv") %>% filter(class == "Core Gene") %>% select("gene")
names(core_gene_set) <- "Row.names"
row.names(deg_CENH3_affected_gene) <- deg_CENH3_affected_gene$Row.names
deg_CENH3_affected_gene[deg_CENH3_affected_gene$pvalue<=0.05,]
